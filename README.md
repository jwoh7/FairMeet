...
### 시연 전날
- [ ] `python scripts/bootstrap_tokens.py` **재실행** (7일 만료)
- [ ] 시연 계정 캘린더에 **그럴듯한 일정을 미리 채움** (빈 캘린더면 협상이 싱겁다)
- [ ] HARD 캘린더에 실제 일정을 넣어 **거부권 발동 장면** 준비
- [ ] 잔액 원장에 과거 이력 주입 (`store.seed_balance`)
- [ ] 전체 흐름 **녹화** (Wi-Fi 실패 대비)
- [ ] `git status` 에 `.env` / `tokens/` / `credentials.json` 없음 확인

### 당일 아침
- [ ] `python scripts/preflight.py --live` 전체 통과
- [ ] 4개 터미널 기동, `/health` 확인
- [ ] 발표장 네트워크에서 Google API 접근 가능한지 (일부 대학망은 차단)
- [ ] 노트북 절전·알림 끄기
...
```
common/       프레임워크 독립 코어 — 외부 의존성 없이 테스트 가능
  fairness.py   ★ 공정성 solver (regret + concession_balance)
  slots.py      슬롯 그리드 (인덱스 = 프로토콜의 슬롯 번호)
  protocol.py   메시지 스키마 — reason 필드가 없는 것이 의도다
  ids.py        그룹별 HMAC 가명

agent/        개인 에이전트 — 개인정보가 머무는 곳
  cost.py         비용 함수 (개인정보 접근하는 유일한 모듈)
  calendar_client.py  ★ FakeCalendar / GoogleCalendar 같은 인터페이스
  negotiator.py   협상 로직 (HTTP 와 분리)
  server.py       FastAPI 껍데기

mediator/     중재자 — 숫자만 본다
  store.py        ★ 상태 머신 (조건부 UPDATE 로 멱등) + 스키마 마이그레이션
  negotiation.py  협상 라운드 루프
  approval_flow.py ★ 거절 → 차순위 재제안 → 종료 (한 트랜잭션), 대시보드 뷰
  tokens.py       서명된 1회용 승인 토큰 (제안 버전·만료 결합)
  notifier.py     ApprovalNotifier: SMTP(TLS) / Console / Fake
  commit.py       ★ 재확인 → 단일 멱등 커밋 → 잔액
  payload_guard.py 허용 필드 검증
  approval_web.py 참가자별 비공개 승인 페이지 (GET 은 상태를 바꾸지 않는다)

eval/
  demo_local.py  오프라인 전 구간 데모
  compare.py     4개 전략 비교 실험
```

---

## 발표에서 해도 되는 말 / 하면 안 되는 말

### ✅ 해도 되는 주장

- "조회에는 **FreeBusy 스코프만** 사용하며, 이 API는 일정 제목을 반환하지 않습니다."
- "코드에서 `events().list`/`get` 을 **한 번도 호출하지 않으며, 정적 테스트로 검증**합니다."
  (`tests/test_no_event_read.py`)
- "중재자가 받는 payload는 **허용 필드 스키마로 강제**되며, 제목·사유 필드는
  스키마에 **존재하지 않습니다**."
- "목적함수가 총합 효율이 아니라 **최대 개인 불이익 최소화 + 누적 부담 보정**입니다."
- "regret을 각자의 최소 비용 기준 상대값으로 정규화하므로, **비용을 일괄
  부풀리는 전략은 구조적으로 무효화**됩니다." (`test_uniform_cost_inflation_has_no_effect`)
- "확정은 **전원 승인 없이는 일어나지 않으며**, 승인 여부는 공개 채널에
  노출되지 않습니다."
- "커밋 직전 전원 캘린더를 재확인하고, **세션 ID 기반 멱등성 키**로
  중복 생성을 막습니다." (`test_exactly_one_event_created`)
- "FreeBusy 오류를 **빈 시간으로 해석하지 않고 fail-closed** 처리합니다."
- "A2A **개념을 차용했으나 1.0 규격 준수는 아니며**, Agent Card에
  `a2aCompliant: false` 를 명시합니다."

### ❌ 하면 안 되는 주장

| 금지 | 이유 |
|---|---|
| "완전 탈중앙 협상" | 중앙 중재자가 전체 비용 벡터를 받는다 |
| "제로 지식" / "프라이버시 보장" | 비용 벡터는 생활 패턴 추론이 가능한 메타데이터다 |
| "API 권한 수준에서 제목을 못 읽는다" | `calendar.events.owned` 도 소유 캘린더 이벤트를 읽을 수 있다 |
| "A2A 호환" | Task 생명주기·보안 스킴·버전 협상 미구현 |
| "privacy leak = 0" | 측정 불가능한 주장이다 |
| "권력 구조를 제거한다" | 의사결정 보조 장치일 뿐이다 |

### 예상 질문

**Q. 중앙 서버가 전부 보는데 왜 에이전트인가요?**
> 맞습니다. MVP는 완전 탈중앙이 아니라 연합형 효용 계산 구조입니다. 에이전트
> 분리의 실익은 두 가지입니다. 첫째, 원본 캘린더 내용과 개인 사유가 프로세스
> 경계를 넘지 않습니다. 둘째, progressive disclosure로 전환할 때 중재자만
> 바꾸면 되도록 설계했습니다.

**Q. Reclaim이나 Clockwise랑 뭐가 다른가요?**
> 목적함수가 다릅니다. 그 제품들은 총 효율을 최적화합니다. 저희는 가장 불리한
> 사람의 불이익을 최소화하고, 과거 누적 부담을 다음 결정에 반영합니다.
> `python -m eval.compare` 로 같은 데이터에서 네 전략을 비교한 결과를 보여드릴 수 있습니다.

**Q. 사용자가 거짓말하면요?**
> 일괄 부풀림은 regret 정규화로 자동 무효화됩니다. 하지만 특정 슬롯만 과장하는
> 공격은 막지 못합니다. 알려진 한계이고, 비용 규칙 버전 검증과 범위 검증으로
> 완화만 했습니다.

**Q. 비용 벡터로 뭘 알 수 있나요?**
> 정직하게 말씀드리면 꽤 많습니다. 매주 같은 시간이 거부권이면 정기 일정의
> 존재가 드러납니다. 제목은 감췄지만 형태는 감추지 못했습니다. 그래서
> "프라이버시 보장"이 아니라 "노출 최소화"라고 말합니다.

---

## 비교 실험 결과

`python -m eval.compare` (3인, 40슬롯, 30라운드, seed=7)

| 전략 | A | B | C | Gini | 총합 | 편차 |
|---|---:|---:|---:|---:|---:|---:|
| 선착순 | 893 | 964 | 947 | 0.017 | 2805 | 71 |
| 합계 최소화 | 363 | 425 | 449 | 0.046 | 1237 | 86 |
| minimax (잔액 없음) | 350 | 443 | 551 | 0.100 | 1343 | 201 |
| **FairMeet** | 469 | 459 | 473 | **0.007** | 1401 | **14** |

**발표에서 쓸 한 문장:**

> "순수 minimax는 Gini 0.100, 여기에 누적 양보 보정을 더하면 0.007입니다.
> 총 부담은 sum-min이 더 낮지만 그 해는 특정 참여자에게 부담이 쏠립니다.
> 저희가 최적화한 것은 효율이 아니라 분배입니다."

단, **FairMeet이 모든 지표에서 최고는 아니다.** 총 부담은 sum-min이 낮다.
트레이드오프를 먼저 인정하는 편이 심사에서 강하다.

---

## 알려진 한계

1. **비용 벡터는 메타데이터다.** 제목은 감췄지만 형태는 감추지 못했다.
   다음 단계는 progressive disclosure (슬롯별 비용 구간만 공개).
2. **특정 슬롯만 과장하는 전략적 행동을 막지 못한다.** 일괄 부풀림은 막힌다.
3. **중앙 중재자 구조다.** 완전 탈중앙이 아니다.
4. **A2A 1.0 규격 미준수.** 개념만 차용했다.
5. **이동시간 미구현.** 출발지가 개인정보이고 Routes API 연동 비용이 크다.
   따라서 SDG 13(기후) 주장은 하지 않는다.
6. **관리자는 한 명의 공용 계정이다.** 관리자 로그인은 v4 에서 생겼지만 사용자별 계정·역할(RBAC)은 없다.
   로그인한 관리자는 모든 회의를 본다. 로그인 세션과 로그인·요청 속도 제한 기록은 서버 메모리에 있어 서버를 재시작하면
   다시 로그인해야 하고 제한 기록도 초기화된다 (프로세스 한 개 기준).
7. **관리자에게는 '응답 완료' 여부가 보인다.** 동의/거절은 구분해 보여주지 않지만,
   거절은 즉시 다음 후보로 넘어가므로 현재 제안에서 '응답 완료'인 사람은 동의한 사람이다.
   다른 참가자의 승인 페이지에는 아무것도 표시되지 않는다.
8. **Slack 승인 경로는 새 흐름에 맞게 수정했지만 실제 Slack 으로는 검증하지 못했다**
   (`slack-bolt` 미설치, 자동 테스트 대상 아님).
9. **승인 정족수는 사람의 '동의'에만 적용된다.** 캘린더 가능 여부는 여전히 참가자 전원이 가능한
   시간만 제안한다 ('나머지는 최소 2명만 가능하면'을 가능 인원 기준으로 풀지는 않는다). 정족수
   회의에서는 선택 참석자의 거절이 재제안을 일으키지 않으므로 7번의 '응답 완료 = 동의' 추론은
   전원 승인 회의에만 성립한다.
10. **일정 변경(재협상)으로 확정된 새 시간의 부담은 양보 잔액에 다시 반영된다.** 이동 전 시간의
    반영분을 되돌리지 않는다. 정족수로 확정된 회의는 기준 인원이 승인하는 즉시 확정되어, 나머지
    초대 대상은 '나만 빠지기'만 요청할 수 있다.
11. **반복 일정·장소·접근성·날씨 조건은 자동 처리하지 않는다.** 표시하고 사람이 확인한다
    (날씨·교통 등은 공급자 인터페이스만 있다). 회의 전후 여유는 에이전트를 새 코드로 다시 시작해야
    자동 처리된다. PDF 분석은 OCR 을 지원하지 않는다. 변경 링크의 서명 키는 승인 링크와 같은
    그룹 비밀키(`FAIRMEET_GROUP_SECRET`)에서 용도별로 파생한다.

12. **입력 검문은 규칙 기반이고 한국어 중심이다.** `mediator/guard.py` 는 여러 신호를 조합하지만 처음 보는 우회 표현을
    모두 잡는다고 보장하지 않는다. 그래서 검문 하나에 의존하지 않고 구조화 추출·서버 재검증·인증·객체 권한·출력 필터를
    함께 쓴다. 애매하면 자동으로 시작하지 않고 사용자에게 보여 준다.
13. **Cloudflare Quick Tunnel 은 임시 시연용이다.** SLA 가 없고 SSE 를 지원하지 않으며 동시 요청 200개 제한이 있다.
    터널을 끄거나 다시 켜면 주소가 바뀌어 이미 보낸 이메일 링크가 열리지 않는다. 고정 주소가 필요하면 이름 있는
    터널(도메인 필요)을 써야 한다.
14. **회의 자료의 규칙 기반 정리는 발췌 수준이다.** LLM 이 없을 때는 키워드로 고른 문장을 쪽 번호와 함께 보여 줄 뿐
    요약·비판·종합을 하지 않는다. OCR 은 지원하지 않는다 (이미지뿐인 PDF 는 거절).
15. **시연 모드 원장에는 참가자 이름이 보인다.** 잔액이 누구의 것인지가 공정성 시연의 핵심이기 때문이다. 협상 과정
    (상태 전이)은 익명이고 개인 regret·일정 제목·거절 사유·이메일은 어디에도 없다.
